...
class UsaditosCarsScraper:
    """Scraper for UsaditosCars.com"""
    
    BASE_URL = "https://usaditoscars.com"
    ...
    async def extract_listing_data(self, listing_element) -> Optional[Dict[str, Any]]:
        ...
        try:
            title_elem = await listing_element.query_selector("h2, h3, .title, .vehicle-title")
            title = await title_elem.inner_text() if title_elem else ""
            
            price_elem = await listing_element.query_selector(".price, .precio, .vehicle-price")
            ...
            if url:
                ...
                if match:
                    external_id = f"UC-{match.group(1)}"
            ...
            brand = None
            model = None
            brands = ['Toyota', 'Chevrolet', 'Ford', 'Jeep', 'Nissan', 'Honda', 'Hyundai', 'Kia']
            for b in brands:
            ...
            print(f"❌ Error extracting UsaditosCars listing: {e}")
            return None
    ...
    async def scrape_page(self, page: Page, page_number: int) -> List[Dict[str, Any]]:
        ...
        print(f"📄 Scraping UsaditosCars page {page_number}: {url}")
        
        try:
            await page.goto(url, wait_until="domcontentloaded", timeout=30000)
            await page.wait_for_timeout(2000)
            
            selectors = [".vehicle", ".car-item", ".product", "article"]
            listings = []
    ...
    async def scrape(self, max_pages: int = 2) -> List[Dict[str, Any]]:
        """Scrape UsaditosCars"""
        print(f"🚗 Starting UsaditosCars scrape...")
        ...
        await page.close()
        print(f"🎉 UsaditosCars scrape complete: {len(all_vehicles)} vehicles")
        return all_vehicles


async def main():
    async with UsaditosCarsScraper() as scraper:
